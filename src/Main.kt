...
// function that has overide, it takes care of the action perfromed from the user by using an action event.
    override fun actionPerformed(e: ActionEvent?) {
        // when that finds the source using e?
        when (e?.source) {
            // when up button is pressed the playey moves -1 on the y-axis and 0 on the x-axis
            upButton -> if (app.movingPlayer(0, -1)) updateView()
            // when down button is pressed the playey moves 1 on the y-axis and 0 on the x-axis
            downButton -> if (app.movingPlayer(0, 1)) updateView()
            // when left button is pressed the playey moves 0 on the y-axis and -1 on the x-axis
            leftButton -> if (app.movingPlayer(-1, 0)) updateView()
            // when right button is pressed the playey moves 0 on the y-axis and 1 on the x-axis
            rightButton -> if (app.movingPlayer(1, 0)) updateView()
            // when the instructionbutton is pressed the jpotionpane is used to show the messahe dialog
            instructionsButton -> JOptionPane.showMessageDialog(
                // uses the parentcomponent "this"
                this,
                // displays the message which can be seen below
